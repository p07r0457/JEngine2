...
public class Field extends java.lang.Object {
  ...
  public static Date timeStampToDate(String timeStamp) {
    ...
    gc.set(gc.YEAR,Integer.parseInt(timeStamp.substring(0,4)));
    if(len>=6) {
      gc.set(gc.MONTH,Integer.parseInt(timeStamp.substring(4,6)));
      
      if(len >=8) {
        gc.set(gc.DAY_OF_MONTH,Integer.parseInt(timeStamp.substring(6,8)));
        
        if(len >=12) {
          gc.set(gc.HOUR,Integer.parseInt(timeStamp.substring(8,10)));
          gc.set(gc.MINUTE,Integer.parseInt(timeStamp.substring(10,12)));
          
          if(len >=14) {
            gc.set(gc.SECOND,Integer.parseInt(timeStamp.substring(12,14)));
          }
